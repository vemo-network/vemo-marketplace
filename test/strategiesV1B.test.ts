...
describe.skip("DareMarket V1B strategies test", () => {
    ...
    beforeEach(async () => {
        ...
        const StrategyStandardSaleForFixedPriceV1B =
            await ethers.getContractFactory(
                "StrategyStandardSaleForFixedPriceV1B"
            );
        strategyStandardSaleForFixedPriceV1B =
            await StrategyStandardSaleForFixedPriceV1B.deploy();
        await strategyStandardSaleForFixedPriceV1B.deployed();

        const StrategyAnyItemFromCollectionForFixedPriceV1B =
            await ethers.getContractFactory(
                "StrategyAnyItemFromCollectionForFixedPriceV1B"
            );
        strategyAnyItemFromCollectionForFixedPriceV1B =
            await StrategyAnyItemFromCollectionForFixedPriceV1B.deploy();
    ...
    describe("#1 - Ownership", async () => {
        it("Owner functions work as expected", async () => {
            ...
            let tx = await strategyStandardSaleForFixedPriceV1B
                .connect(admin)
                .setProtocolFee("100");
            await expect(tx)
                .to.emit(strategyStandardSaleForFixedPriceV1B, "NewProtocolFee")
                .withArgs("100");

            tx = await strategyAnyItemFromCollectionForFixedPriceV1B
                .connect(admin)
                .setProtocolFee("100");
            await expect(tx)
                .to.emit(
