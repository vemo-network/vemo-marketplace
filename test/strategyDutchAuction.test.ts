...
describe.skip("Strategy - Dutch Auction", () => {
    ...
    it("ERC1155 - Buyer overpays", async () => {
        ...
        await expect(tx)
            ...
            .withArgs(
                ...
                takerBidOrder.price.mul(protocolFee).div("10000")
            );

        // User 2 had minted 2 tokenId=1 so he has 4
        const balanceTokenId1 = await mockERC1155.balanceOf(
            takerBidUser.address,
            "1"
        );
        expect(balanceTokenId1).to.be.deep.equal(BigNumber.from("4"));
    });
    ...
    it("Min Auction length creates revertion as expected", async () => {
        await expect(
            strategyDutchAuction
                .connect(admin)
                .updateMinimumAuctionLength("899")
        ...
        await expect(
            StrategyDutchAuction.connect(admin).deploy("900", "899")
        ).to.be.revertedWith("Owner: Auction length must be > 15 min");
    });

    it("Owner functions work as expected", async () => {
        const tx = await strategyDutchAuction
            .connect(admin)
            .updateMinimumAuctionLength("1000");
        await expect(tx)
    ...
    it("Owner functions are only callable by owner", async () => {
        ...
        await expect(
            strategyDutchAuction
                ...
                .updateMinimumAuctionLength("500")
